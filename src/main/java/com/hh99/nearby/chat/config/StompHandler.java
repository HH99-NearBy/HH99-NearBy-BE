...
public class StompHandler implements ChannelInterceptor {
    ...
    public Message preSend(Message<?> message, MessageChannel channel) {
        ...
        if (command.compareTo(StompCommand.DISCONNECT) == 0) {
            ...
            Optional<MemberChallenge> memberChallenge = memberChallengeRepository.findByMember_Id(member.get().getId()); //MemberId로 참여한 첼린지 데이터 찾기
            Long endTime = System.currentTimeMillis(); // 사용자 연결해제 시간
            Long realTime = (endTime - chat.getEntryTime()) / 1000; // 연결해제 시간 (초단위) 분단위로 바꿔야함
